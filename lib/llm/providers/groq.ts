...
export class GroqProvider implements LLMProvider {
    ...
    constructor(model: string = 'llama3-70b-8192') {
        ...
        if (!this.apiKey) {
            console.warn('GROQ API key not found. GROQ provider will not function properly.');
        }
    }
    ...
    private async makeRequest(options: LLMRequestOptions): Promise<any> {
        ...
        // Convert to the format expected by GROQ API (similar to OpenAI format)
        const apiMessages = [
        ...
        if (useSearch && messages.length > 0 && messages[messages.length - 1].role === 'user') {
            ...
            try {
                ...
                console.error('Error during GROQ search:', error);
    ...
    async *generateStreamingResponse(options: LLMRequestOptions): AsyncGenerator<LLMResponseChunk, void, unknown> {
        try {
            ...
            console.error('Error in streaming response from GROQ:', error);
    ...
    async searchInternet(query: string): Promise<string> {
        try {
            ...
            console.error('Error searching internet with GROQ:', error);
